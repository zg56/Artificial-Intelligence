...
public class OthelloState {
    ...
    public int modifiedEvaluation() {
        //Add weight to the score if the new state has corners for the player
        int score = 0;
        for(int i = 0; i < boardSize; i++){
            for(int k = 0; k < boardSize; k++){
                if(board[i][k]== PLAYER1){
                    score++;
                } 
                if(board[i][k]== PLAYER2){
                    score--;
                } 
            }
        }

        // check for corners on the board
        for(int i = 0; i < boardSize; i += boardSize-1){
            for(int k = 0; k < boardSize; k += boardSize-1){
                if(board[i][k]== PLAYER1){
                    score += 4;
                } 
                if(board[i][k]== PLAYER2){
